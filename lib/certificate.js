...
//  (nom, cours...), cercles (sceau). Avantage : net à toute taille, léger,
//  et entièrement personnalisable pour chaque étudiant.
...
// (ex: le "&" devient "&amp;"). Sécurité contre les injections.
function esc(s) {
  return String(s || "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
...
export function renderCertificateSVG({
  ...
  const name = esc(clamp(student || "Student Name", 34)); // nom de l'étudiant
  const courseName = esc(clamp(course || "Course Name", 46)); // nom du cours
  const issuerName = esc(clamp(issuer || "Academy", 30)); // émetteur
  const dateStr = esc(clamp(date || "", 24)); // date
  // Le pied de page : Token + Serial + "Hedera Testnet" (on ignore les champs vides).
  const footer = esc(
    [
  ...
  <g transform="translate(500 555)">
    ...
    <path d="M-13 44 L-19 66 L-7 59 L0 72 L7 59 L19 66 L13 44 Z" fill="url(#ribbon)" stroke="#1a2340" stroke-width="1"/>
  </g>

  <text x="500" y="644" text-anchor="middle" font-size="11" fill="#8a856f" letter-spacing="1">${footer}</text>
</svg>`;
}
